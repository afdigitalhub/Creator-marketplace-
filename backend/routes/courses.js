...
async function courseStructure(
  ...
  const totalLessons =
    lessonsResult.rows.length;

  const completedLessons =
    lessonsResult.rows.filter(
      lesson =>
        progressMap.get(
          String(lesson.id)
        )?.completed
  ...
  return {
    course: {
      ...
      affiliate_commission_percent:
        Number(
          course.affiliate_commission_percent || 50
        ),
      ...
      const totals =
        await pool.query(
          `SELECT
             ...
             COUNT(
               DISTINCT cal.course_id
             )::int AS courses_promoted
      ...
      const sales =
        await pool.query(
          `SELECT
             ...
             COALESCE(
               SUM(o.affiliate_amount),
               0
             ) AS commission_from_orders
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      console.error(
        "Resolve affiliate link error:",
        err
      );

      res.status(500).json({
        error:
          "Could not resolve affiliate link"
      });
    }
  }
);
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const affiliate_url =
        `${origin}/course-details.html` +
        `?id=${encodeURIComponent(course.id)}` +
      ...
      res.json({
        ...
        commission_percent:
          Number(
            course.affiliate_commission_percent ||
            50
          ),
  ...
  async (req, res) => {
    try {
      ...
      if (!enrollment) {
        return res.status(403).json({
          error:
            "Purchase this course to access the lessons"
        });
      }
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      res.status(500).json({
        error:
          "Could not load course inventory"
      });
    }
  }
);
